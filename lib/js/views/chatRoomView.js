...
exports.chatRoomView = Backbone.View.extend ({
  ...
  template: _.template (`
    ...
    <form>
      <input name="message-holder" type="text" placeholder="Type ..."/>
  ...
  _initListeners: function () {
    ...
    this.listenTo(this.model.players, 'add', this._playerLeft)
  ...
  sendMessage: function (e) {
    e.preventDefault()
    console.log(`sending message to: ${this.model.get('channelID')}`)
    socket.broadcastTo({ tunnel: this.model.get('channelID'), data: { message: this.inputMessage.val()} })
